...
var compile = function(object, block, fns, startingPosition, inputs, types, used) {
	...
	if (block[0] === 'forward:') {
		...
		source += 'if (S.node) S.node.gain.setValueAtTime(S.volume, P.audioContext.currentTime);\n';
		source += 'for (var sounds = S.sounds, i = sounds.length; i--;) {\n';
		source += '  var sound = sounds[i];\n';
		source += '  if (sound.node && sound.target === S) {\n';
		source += '    sound.node.gain.setValueAtTime(S.volume, P.audioContext.currentTime);\n';
		source += '  }\n';
		source += '}\n';
